...
static int dns_transaction_prepare(DnsTransaction *t, usec_t ts) {
        ...
        if (set_isempty(t->notify_zone_items)) {
                ...
                r = dns_cache_lookup(&t->scope->cache, t->key, t->dnssec_mode != DNSSEC_NO,
                                     &t->answer_rcode, &t->answer, &t->answer_authenticated);
                if (r < 0)
                        return r;
                if (r > 0) {
...
static int dns_transaction_validate_dnskey_by_ds(DnsTransaction *t) {
        ...
        DNS_ANSWER_FOREACH_IFINDEX(rr, ifindex, t->answer) {
                ...
                r = dns_answer_add_extend(&t->validated_keys, rr, ifindex, DNS_ANSWER_AUTHENTICATED, DNSSEC_VALIDATED);
                if (r < 0)
                        return r;
        }
