...
#define _DNS_ANSWER_FOREACH_FLAGS_RESULT(q, kk, fl, dr, a)              \
        for (unsigned UNIQ_T(i, q) = ({                                 \
                                (kk) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].rr : NULL; \
                                (fl) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].flags : 0; \
                                (dr) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].dnssec_result : 0; \
                                0;                                      \
                        });                                             \
             (a) && (UNIQ_T(i, q) < (a)->n_rrs);                        \
             UNIQ_T(i, q)++,                                            \
                     (kk) = ((UNIQ_T(i, q) < (a)->n_rrs) ? (a)->items[UNIQ_T(i, q)].rr : NULL), \
                     (fl) = ((UNIQ_T(i, q) < (a)->n_rrs) ? (a)->items[UNIQ_T(i, q)].flags : 0), \
                     (dr) = ((UNIQ_T(i, q) < (a)->n_rrs) ? (a)->items[UNIQ_T(i, q)].dnssec_result : 0))

#define DNS_ANSWER_FOREACH_FLAGS_RESULT(kk, flags, dnssec_result, a) _DNS_ANSWER_FOREACH_FLAGS_RESULT(UNIQ, kk, flags, dnssec_result, a)

#define _DNS_ANSWER_FOREACH_FULL(q, kk, ifi, fl, dr, a)                 \
        for (unsigned UNIQ_T(i, q) = ({                                 \
                                (kk) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].rr : NULL; \
                                (ifi) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].ifindex : 0; \
                                (fl) = ((a) && (a)->n_rrs > 0) ? (a)->items[0].flags : 0; \
...
#define DNS_ANSWER_FOREACH_FULL(kk, ifindex, flags, dnssec_result, a) \
        _DNS_ANSWER_FOREACH_FULL(UNIQ, kk, ifindex, flags, dnssec_result, a)
