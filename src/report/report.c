...
static int metrics_call(Context *context, const char *name, const char *path) {
        ...
        const char *method =
                context->action == ACTION_LIST           ? "io.systemd.Metrics.List" :
                context->action == ACTION_DESCRIBE       ? "io.systemd.Metrics.Describe" :
                context->action == ACTION_LIST_FACTS     ? "io.systemd.Facts.List" :
                context->action == ACTION_DESCRIBE_FACTS ? "io.systemd.Facts.Describe" :
                assert_not_reached();
