...
static void test_merge_env_file(void) {
        ...
        r = merge_env_file(&a, NULL, t, MERGE_ENV_FILE_EXPAND);
        assert_se(r >= 0);
        strv_sort(a);
        ...
        assert_se(streq(a[0], "one=2"));
        assert_se(streq(a[1], "twelve=${one}2"));
        assert_se(streq(a[2], "twentyone=2${one}"));
        assert_se(streq(a[3], "twentytwo=2${one}"));
        assert_se(a[4] == NULL);

        strv_free(a);
        a = strv_new("one=4", NULL);
