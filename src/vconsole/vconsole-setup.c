...
static int toggle_utf8_sysfs(bool utf8) {
        ...
        if (r < 0)
                log_warning_errno(r, "Failed to %s sysfs UTF-8 flag: %m", enable_disable(utf8));
 ...
 */
static void setup_remaining_vcs(int fd, unsigned n_setup_vts, bool utf8) {
...
int main(int argc, char **argv) {
        ...
        if (font_copy)
                log_info("Setting up %s with %s,%s %s,%s,%s and copying to %u consoles",
                         vc,
                         strdash(vc_keymap), strdash(vc_keymap_toggle),
                         strdash(vc_font), strdash(vc_font_map), strdash(vc_font_unimap),
                         n_setup_vts - 1);
        else
                log_info("Setting up %s with %s,%s %s,%s,%s",
                         vc,
                         strdash(vc_keymap), strdash(vc_keymap_toggle),
                         strdash(vc_font), strdash(vc_font_map), strdash(vc_font_unimap));
        ...
        if (font_copy) {
                if (font_ok)
                        setup_remaining_vcs(fd, n_setup_vts, utf8);
                else
